...
inherit fcaps meson optfeature lua-single
...
if [[ ${PV} == 9999 ]]; then
	inherit git-r3
	EGIT_REPO_URI="https://github.com/dawsers/${PN}.git"
else
	MY_PV=${PV/_rc/-rc}
	SRC_URI="https://github.com/dawsers/${PN}/archive/${MY_PV}.tar.gz -> ${P}.tar.gz"
...
LICENSE="MIT"
SLOT="0"
IUSE="+man +scrollbar +scrollnag tray wallpapers X"
REQUIRED_USE="tray? ( scrollbar ) ${LUA_REQUIRED_USE}"
...
DEPEND="
	...
	x11-libs/pixman
	media-libs/libglvnd
	scrollbar? ( x11-libs/gdk-pixbuf:2 )
	tray? ( || (
		sys-apps/systemd
		sys-auth/elogind
		sys-libs/basu
...
src_configure() {
	local emesonargs=(
		...
		$(meson_feature tray)
		$(meson_feature scrollbar gdk-pixbuf)
		$(meson_use scrollnag)
		$(meson_use scrollbar)
