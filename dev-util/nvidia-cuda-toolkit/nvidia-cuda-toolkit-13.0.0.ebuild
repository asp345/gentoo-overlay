...
src_install() {
	...
	cd "${D}${CUDA_PATH}" || die "Failed to cd into ${D}${CUDA_PATH}"
	dosym -r "${CUDA_PATH}/targets/${narch}-linux/include" "include"
	dosym -r "${CUDA_PATH}/targets/${narch}-linux/lib" "$(get_libdir)"
